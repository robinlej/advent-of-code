    ...
    input.forEach((row, y) => {
        row.forEach((cell, x) => {
            if (cell === "*") {
                ...
                numbers.push(...possibleNumbers[y]);
                if (y > 0) numbers.push(...possibleNumbers[y - 1]);
                if (y < input.length - 1) numbers.push(...possibleNumbers[y + 1]);
                numbers = numbers.filter(number =>
                    number.index.includes(x) || number.index.includes(x-1) || number.index.includes(x+1)
                );
                if (numbers.length > 1) {
                    let product = 1;
                    numbers.forEach(number => product *= parseInt(number.number));
                    total += product;
                }
            }
        });
