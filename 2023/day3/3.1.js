const input = `.679.....662....71............................805..........862.680...................................................................687....
...
function getTotal(input) {
    ...
    input.forEach((row, y) => {
        ...
        let cursor = 1;
        for (let cellIndex = 0; cellIndex < row.length; cellIndex += cursor) {
            let cell = row[cellIndex];
            cursor = 1;
            if (cell.match(/[0-9]/)) {
                let nr = { number: cell, index: [cellIndex] };
                while (row[cellIndex + cursor]?.match(/[0-9]/)) {
                    const nextCellIndex = cellIndex + cursor;
                    nr.number = `${nr.number}${row[nextCellIndex]}`;
                    nr.index.push(nextCellIndex);
                    cursor++;
                }
                possibleNumbers[y].push(nr);
            }
        }
    });

    possibleNumbers.forEach((row, y) => {
        row.forEach(number => {
            const firstIndex = number.index[0];
            const lastIndex = number.index[number.index.length - 1]
            const currRow = input[y];
            const prevRow = input[y - 1];
            const nextRow = input[y + 1];
        
            const cellsToCheck = [];
            function addCellsToCheck(row) {
                if (firstIndex > 0) cellsToCheck.push(row[firstIndex - 1]);
                if (lastIndex < row.length - 1) cellsToCheck.push(row[lastIndex + 1]);
            }

            addCellsToCheck(currRow);
            if (prevRow) {
                addCellsToCheck(prevRow);
            }
            if (nextRow) {
                addCellsToCheck(nextRow);
            }
            number.index.forEach(idx => {
                if (prevRow) {
                    cellsToCheck.push(prevRow[idx]);
                }
                if (nextRow) {
                    cellsToCheck.push(nextRow[idx]);
                }
            });

            if (cellsToCheck.some(cell => !cell.match(/[0-9.]/))) {
