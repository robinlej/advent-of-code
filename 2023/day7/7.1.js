...
const CARDS_VALUES = { A: 14, K: 13, Q: 12, J: 11, T: 10, 9: 9, 8: 8, 7: 7, 6: 6, 5: 5, 4: 4, 3: 3, 2: 2 };
...
function getType(hand) {
    if (hand.every(card => card === hand[0])) return "five";
    ...
    if (values.some(val => val === 4)) return "four";

    let hasThree = hasPair = false;
    if (values.some(val => val === 3)) hasThree = true;
    if (values.some(val => val === 2)) hasPair = true;
    if (hasThree && hasPair) return "fullHouse";
    if (hasThree) return "three";
    if (hasPair && values.findIndex(val => val === 2) !== values.findLastIndex(val => val === 2)) return "pairs";
